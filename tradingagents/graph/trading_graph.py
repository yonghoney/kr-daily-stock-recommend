...
class TradingAgentsGraph:
    ...
    def _run_graph(self, company_name, trade_date, asset_type: str = "stock"):
        ...
        # Optional broker execution (paper / KIS) after PM decision.
        if self.config.get("execution_enabled"):
            try:
                from tradingagents.execution.gateway import ExecutionGateway

                gateway = ExecutionGateway(self.config)
                exec_result = gateway.execute_from_decision(
                    ticker=company_name,
                ...
                final_state["execution_result"] = {
                    ...
                    "rating": exec_result.rating,
                    "message": exec_result.message,
                    "details": exec_result.details,
                    "order_id": (
                        exec_result.order_result.order_id
                        if exec_result.order_result
                        else None
                    ),
                }
                logger.info(
                    "Execution for %s: executed=%s skipped=%s msg=%s",
                    company_name,
                ...
                logger.exception("Execution gateway failed for %s", company_name)
                final_state["execution_result"] = {
                    "executed": False,
                    "skipped": True,
                    "rating": None,
