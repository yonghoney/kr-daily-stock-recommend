...
def normalize_symbol(raw: str) -> str:
    ...
    Resolution order (first match wins):
      ...
      2. Korean equity: 6-digit KRX codes → ``.KS`` / ``.KQ``.
    ...
    # Korean 6-digit codes before forex: digits are never ISO currency pairs.
    from tradingagents.dataflows.kr_symbols import is_kr_equity, normalize_kr_symbol

    if is_kr_equity(s):
        return normalize_kr_symbol(s)
