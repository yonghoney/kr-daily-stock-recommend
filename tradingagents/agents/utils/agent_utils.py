...
def get_language_instruction() -> str:
    ...
    if lang.strip().lower() != "english":
        parts.append(f"Write your entire response in {lang}.")
    if str(cfg.get("market", "")).lower() == "kr":
        parts.append(
            "Context: Korean equity (KRX). Prefer KRW quotes, Korean disclosures "
            "(DART), trading-unit/lot conventions, and domestic market hours "
            "(09:00–15:30 KST). Mention transaction tax / fees when discussing sells."
        )
    if not parts:
        return ""
    return " " + " ".join(parts)
